...
<!-- Main content -->
        ...
        <div class="card-body">
        <?=form_open('group/update/'.$get_group->id, array('name'=>'form_group', 'method'=>'post'))?>
        <div class="form-group">
        <?=form_label('Name');?>
        <?=form_input(array('name'=>'name', 'class'=>'form-control', 'value'=>"$get_group->name",'type'=>'text', 'id'=>'name', 'placeholder'=>"Enter Your Group Name"));?>
        ...
        <option value="1" <?=($get_group->is_active == '1')? 'selected':'' ?>>Active</option>
        <option value="0" <?=($get_group->is_active == '0')? 'selected':'' ?>>Inactive</option>
        </select>
        </div>
